...
CHAMELEON_DATASET_RELATIVE_PATH = ""

class ChameleonTorchDataset(Dataset):
...
class ChameleonDataModule(LightningDataModule):
    def __init__(self,
                 dataset_path: str,
                 batch_size=32,
                 random_state=42,
    ...
    def setup(self):
        ...
        self.train_dataset = ChameleonTorchDataset(self.node_features,
                                               self.node_labels,
                                               self.train_edge_index,
                                               self.train_mask[:, 0])

        self.val_dataset = ChameleonTorchDataset(self.node_features,
                                             self.node_labels,
                                             self.train_edge_index,
                                             self.val_mask[:, 0])

        self.test_dataset = ChameleonTorchDataset(self.node_features,
                                              self.node_labels,
                                              self.train_edge_index,
                                              self.test_mask[:, 0])
