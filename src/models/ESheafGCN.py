...
class ESheafGCN_content_features(pl.LightningModule):
    def __init__(self,
        ...
        # self.user_feature_embeddings = torch.empty(sum(self.dataset.embed_layers_config['users'], latent_dim))
        # self.item_feature_embeddings = torch.empty(sum(self.dataset.embed_layers_config['items'], latent_dim))

        self.embedding = nn.Embedding(
            sum(self.dataset.embed_layers_config['users'] + self.dataset.embed_layers_config['items']),
            latent_dim
        )
    ...
    def init_parameters(self):
        ...
        # nn.init.normal_(self.user_feature_embeddings, std=0.1)
        # nn.init.normal_(self.item_feature_embeddings, std=0.1)
        nn.init.normal_(self.embedding.weight, std=0.1)
    ...
    def forward_(self, adj_matrix, emb0):
        xmap, y1, rmat, smat, smat_proj = self.sheaf_conv1(adj_matrix, emb0, True)
        y2 = self.sheaf_conv2(adj_matrix, y1, False)
        y3 = self.sheaf_conv3(adj_matrix, y2, False)

        return xmap, y1, y3, rmat, smat, smat_proj
    ...
    def configure_optimizers(self):
        # all_params = [self.user_feature_embeddings, self.item_feature_embeddings]
    ...
    def encode_minibatch(self, users, pos_items, neg_items, edge_index):
        emb0 = self.eval_emb()
        xmap, y, out, rmat, smat, smat_proj = self.forward_(edge_index, emb0)
        # debug_print_tensor(y, "Y")
        # debug_print_tensor(pos_items, "pos_items")
        # debug_print_tensor(neg_items, "neg_items")
        # print(max(pos_items))
    ...
    def eval_emb(self):
        # self.embedding.weight.to('cpu')
        
        users_raw = self.dataset.raw_embeds['users'].to('cuda')
        users_emb = []
        for i, cur_feat_idxs in enumerate(users_raw.T.split(1)):
            shift = sum(self.dataset.embed_layers_config['users'][:i])
            # print(cur_feat_idxs)
            users_emb.append(self.embedding(cur_feat_idxs[0] + shift))
        users_emb = torch.stack(users_emb).mean(dim=0)
        
        items_raw = self.dataset.raw_embeds['items'].to('cuda')
        years_embeds = self.embedding(items_raw[:, 0] + sum(self.dataset.embed_layers_config['users']))
        num_genres = self.dataset.embed_layers_config['items'][-1]
        genres_embeds = torch.matmul(items_raw[:, 1:].to(torch.float), self.embedding.weight[-num_genres:]) / items_raw[:, 1:].sum(dim=1).unsqueeze(1)
        # print(genres_embeds.shape)
        # exit()
        items_emb = torch.stack([years_embeds, genres_embeds]).mean(dim=0)

        return torch.vstack([users_emb, items_emb])
